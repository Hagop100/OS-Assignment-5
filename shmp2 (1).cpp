...
#include <stdio.h>
#include <memory.h>
#include <semaphore.h> //will allow us to utilize semaphores in this program
#include <fcntl.h>
...
int main(int argc, char *argv[])
{
     ...
     sem_t *sem; //declare semaphore variable that will hold pointer to semaphore

     //sem_open will create a semaphore
     //first parameter will be a name for the semaphore
     //second parameter is the oflag and O_CREAT will create a new semaphore if one
     //with that name does not yet exist
     //third parameter gives the semaphore read and write access for other processes
     //fourth parameter initializes the semaphore value to 1
     sem = sem_open("SEMAPHORE", O_CREAT, 0666, 1);

     //if the semaphore was unable to be created it will return SEM_FAILED
     ...
     sem_unlink("SEMAPHORE");//will remove the semaphore name but not destroy it
     sem_close(sem); //will destroy the semaphore and free any allocated memory used by its creation
}
